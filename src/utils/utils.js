...
export function setToken(token) {
  ...
  cookie.save("refresh", token, { path: "/", expires });
}
...
// for translation
export const translateAndRedirect = async (
  dispatch,
  path,
  location,
  changeLang
  ...
  //   localStorage.clear();
  await localStorage.setItem("pathname", path);
  await dispatch(changeLang("en"));
  await location.push(path);
};
